...
class Server(QObject):
    ...
    def run(self):
        ...
        while True:
            ...
            # 开启接收线程
            crthread = Thread(target=self.crec, args=(clientsocket, addr, cq))
            crthread.daemon = True  # 设置随主线程退出
            crthread.start()    # 开始运行

            # 开启发送线程
            csthread = Thread(target=self.csend, args=(clientsocket, addr, cq))
    ...
    def csend(self, clientsocket, addr, q):
        while True:
            ...
            if data['type'] == 'dwnf':    # 用户请求下载文件
                ...
                self.ensend(clientsocket, data)
                break

            else:
                self.ensend(clientsocket, data)    # 否则，就将接收的命令发送给客户端
    ...
    def lg(self, cnt, q):
        ...
        if self.cnum > self.num:
            ...
            db = sqlite3.connect('accounts.db')  # 连接数据库
            cursor = db.cursor()
    ...
    def deread(self, c):
        ...
        print("server receive len:", head_len)
        # 接收报头
